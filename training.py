...
def load_processed_data(data_path: str | None = None) -> tuple[pd.DataFrame, pd.Series]:
    ...
    if target not in df.columns:
        raise ValueError(f"Columna '{target}' no encontrada en {data_path}")
...
def train_all_models(
    ...
    for name in models_to_train:
        if name not in MODEL_REGISTRY:
            print(f"  Modelo '{name}' no registrado, saltando.")
            continue
        print(f"\nEntrenando {name}...")
...
def save_model(pipeline: Pipeline, name: str, metrics: dict, model_dir: str | None = None) -> str:
    ...
    meta = {"model": name, "timestamp": ts, "metrics": metrics, "file": filename}
...
if __name__ == "__main__":
    ...
    train_best_and_save(X_train, y_train, X_test, y_test)
